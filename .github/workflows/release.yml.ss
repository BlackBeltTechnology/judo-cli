    ...
    steps:
      ...
      - name: Clean up existing release artifacts if any
        run: |
          ...
          # Use GitHub API directly instead of GitHub CLI
          # Check if release exists using API
          ...
          if echo "$response" | grep -q '"id":'; then
            ...
            release_id=$(echo "$response" | grep '"id":' | head -1 | awk '{print $2}' | tr -d ',')

            # Delete the release using API
            curl -s -X DELETE \
      ...
      - name: Update VERSION file
        run: |
          ...
          # Note: We don't commit here because the tag should already point to a commit
          # with the correct version. If the version doesn't match, it's an error.
          if git diff --quiet VERSION; then
            echo "VERSION file unchanged - good"
          else
            echo "::error::VERSION file changed after tag creation. Tag should point to commit with correct version."
            echo "Current version in file: $(cat VERSION)"
